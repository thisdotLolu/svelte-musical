...
export const load:PageLoad=async({fetch: _fetch,parent})=>{
    ...
    const [newRealeasesRes, featuredPlaylistsRes, userPlaylistsRes, ...randomCatsRes] = await Promise.all([newReleases, featuredPlaylists, userPlaylists, ...randomCatsPromises]);
